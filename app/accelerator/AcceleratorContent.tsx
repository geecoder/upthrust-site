...
import { getPricing, formatPrice, REGION_LABELS, REGION_PROCESSOR, type Region } from '@/lib/config';
import { useRegion } from '@/lib/useRegion';
import { RHYTHM } from '@/lib/weekly-loop-content';
import { Reveal } from '@/components/Reveal';

// Accelerator-page-specific "market moved from credentials to capability"
// shift rows. Distinct from the About page's shifts (lib/why-now-content.ts)
// even though the current statistics happen to read the same.
const SHIFT = [
...
const PHASES = [
  {
    label: 'Foundation', range: 'WEEKS 0–3', weeks: [
      { num: 'W00', title: 'Onboarding & diagnostic', desc: 'Baseline assessment, tool setup, pathway confirmation, community induction.' },
      { num: 'W01', title: 'Digital product foundations', desc: 'How real product teams work — PM, BA, Design, Engineering, QA, Marketing, Ops, and how they hand work over.' },
      { num: 'W02', title: 'Problem discovery', desc: 'Defining user and business problems clearly before jumping to solutions.' },
      { num: 'W03', title: 'Product strategy & business context', desc: 'Connecting problems to business goals, MVP scope, and why this and not something else.' },
    ],
  },
  {
    label: 'Core skills', range: 'WEEKS 4–7', weeks: [
      { num: 'W04', title: 'Requirements & solution definition', desc: 'Turning a framed problem into something a delivery team can actually build.' },
      { num: 'W05', title: 'Process & experience mapping', desc: 'Making the current and future state visible, and finding where it breaks.' },
      { num: 'W06', title: 'Documentation that survives delivery', desc: 'Writing the artefact your team still trusts in week nine.' },
      { num: 'W07', title: 'Data, metrics & instrumentation', desc: 'Deciding what to measure before you ship, not after.' },
    ],
  },
  {
    label: 'Delivery', range: 'WEEKS 8–11', weeks: [
      { num: 'W08', title: 'Working inside a delivery team', desc: 'Ceremonies, trade-offs, and holding the line on scope.' },
      { num: 'W09', title: 'Quality, testing & sign-off', desc: 'Proving it works before anyone else has to find out that it does not.' },
      { num: 'W10', title: 'Launch & go-to-market', desc: 'Getting it into the world, and reading what happens next.' },
      { num: 'W11', title: 'Portfolio & interview readiness', desc: 'Turning twelve weeks of work into a case study and a story bank.' },
    ],
  },
  {
    label: 'Capstone', range: 'WEEK 12', weeks: [
      { num: 'W12', title: 'Capstone defence', desc: 'You present to a panel and answer for every decision. This is where the credential is earned.' },
    ],
  },
];
...
const ACCEL_FAQ = [
  ...
  { q: 'Do you offer payment plans?', a: 'Yes — in full or across two payments. Each programme page shows exactly what every option costs in your currency.' },
];
...
export default function AcceleratorContent({ initialRegion }: { initialRegion: Region }) {
  ...
  return (
    <>
      ...
      <Reveal style={{ borderBottom: '1px solid var(--border-soft)', background: 'var(--paper)' }}>
        <div className="container" style={{ padding: '96px 24px' }}>
          <div className="stack-mobile" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 64, alignItems: 'end' }}>
            <div>
          ...
          <div className="accel-2col-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 1, background: 'var(--border-soft)', border: '1px solid var(--border-soft)', margin: '44px 0 0' }}>
            <div style={{ background: 'var(--paper)', padding: '32px 30px' }}>
              ...
              {[
                ...
                ['Stuck in product-adjacent roles', 'You already do the work. The Passport helps your title catch up to your reality.'],
                ['Early-career, filtering out', 'Evidence replaces the "3 years experience required" catch-22.'],
                ['People who will do the work', 'Every session. Every assignment. Every revision.'],
              ].map(([title, desc]) => (
            ...
            <div style={{ background: 'var(--paper-dim)', padding: '32px 30px' }}>
              ...
              {[
                ...
                ['You cannot commit 8–10 hours', 'Partial engagement produces weak portfolios. Weak portfolios do not impress employers.'],
      ...
      <Reveal style={{ borderBottom: '1px solid var(--border-soft)', background: 'var(--bone-dim)' }}>
        <div className="container" style={{ padding: '96px 24px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24, marginTop: 20, fontSize: 13, color: 'var(--fg-3)', flexWrap: 'wrap' }}>
            ...
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '0.06em' }}>PROCESSED VIA {REGION_PROCESSOR[region].toUpperCase()}</span>
          </div>
        </div>
      </Reveal>
      ...
      {/* FAQ */}
      <Reveal style={{ background: 'var(--paper)' }}>
